...
def test(model, dataset, batch_size, device='cpu'):
    ...
    print("test set accuracy {:.3f}; preicision:{:.3f}; recall:{:.3f}."
          .format(corr.item()/len(dataset), precision, recall))
...
def main():
    model = graph_cage(30,256,2)
    with open("amine2aldehyde3.ckpt", "rb") as f:
        data = pickle.load(f)
    train(model, data, 64, 150)
